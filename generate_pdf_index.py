...
<head>
    ...
    <title>Lakshman's Sloka Collection</title>
    <style>
        body {{
...
{pdf_items}
    ...
    if pdf_files:
        for pdf_file in pdf_files:
            try:
                # Get file size
                file_size = os.path.getsize(os.path.join("src", pdf_file))
...
"""
        shutil.copyfile(os.path.join("src", pdf_file), os.path.join("docs", pdf_file))
    else:
    ...
    if pdf_files:
        ...
        for pdf in pdf_files:
            size = format_file_size(os.path.getsize(os.path.join("src", pdf)))
            print(f"   • {pdf} ({size})")
    else:
